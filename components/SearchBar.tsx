...
const ResultItem = ({ index, style, data }: ResultItemProps) => {
  ...
  const cleanContent = result.message.content.replace(/<\/?[^>]+(>|$)/g, "").trim()
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-xs text-gray-500 mb-1">
        <span>{result.message.displayName}</span>
        <span>•</span>
      ...
      <div className="text-sm">
        {highlightText(cleanContent, result.matches)}
      </div>
    </div>
  )
}
...
export function SearchBar({ onResultClick }: SearchBarProps) {
  ...
  const handleScroll = ({ scrollOffset, scrollUpdateWasRequested }: { scrollOffset: number, scrollUpdateWasRequested: boolean }) => {
    if (!scrollUpdateWasRequested && hasMore && !isLoading) {
      const totalHeight = (results?.length || 0) * RESULT_ITEM_HEIGHT
      const threshold = 100 // pixels before the end
      
      if (totalHeight - scrollOffset < threshold) {
        // Handle loading more results if needed
      }
    }
  }
        ...
        <div className="w-full max-w-2xl relative">
          <form onSubmit={handleSubmit} className="flex gap-2">
            <div className="relative flex-1">
              {isLoading ? (
                <div role="status" className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2">
                  <Spinner className="h-4 w-4" aria-label="Loading" />
                </div>
              ) : (
